...
module.exports = (req, res, next) => {
  try {
    ...
    const decodedToken = jwt.verify(token, config.secret);//pass the token and the secret string
    //creating a new field that does not yet exist (JavaScript Object containing the email and the UserId)
    req.userData = { userId: decodedToken.id }; //creating this object means that any middleware that runs after this middleware can access this Object (but not those that run before)
    next();
  }
  catch (error) {
